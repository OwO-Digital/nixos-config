{ pkgs, lib, config, ... }:
let
  cfg = config.octelly.programs.neovim;
in
with lib; {
  options.octelly.programs.neovim =
    {
      enable = mkEnableOption "Octelly's Neovim settings";
    };
  ...
  config.programs.nixvim = mkIf cfg.enable {
    ...
    extraConfigLua = ''
      ...
      vim.o.guifont = "${builtins.elemAt config.fonts.fontconfig.defaultFonts.monospace 0}:h12"
