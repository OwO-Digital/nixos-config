{ pkgs, config, lib, ... }:
let
  cfg = config.octelly.desktopEnvironments.plasma;
...
in
with lib; {
  options.octelly.desktopEnvironments.plasma = {
    enable = mkEnableOption "Octelly's Plasma module";
  };
    ...
    programs.plasma = {
      ...
      workspace = {
        cursor = {
          size = 32;
          theme =
            mkIf (hasAttrByPath [ "home" "pointerCursor" "name" ] config)
            ( _: (config.home.pointerCursor.name) null);
        };
      ...
      configFile = {
        ...
        plasma-localerc.Translations.LANGUAGE = mkIf (config.home.language.base != null)
          ( _: ((builtins.elemAt
